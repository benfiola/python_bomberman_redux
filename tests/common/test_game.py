...
class TestGameSuite:
    ...
    def test_process_move_out_of_board(self, empty_game):
        ...
        time.sleep(movement_duration/4)
        empty_game.process()
        assert -0.5 < player_entity.physical_location[0] < old_location[0]
        assert player_entity.is_moving is True

        time.sleep(movement_duration/2)
        empty_game.process()
        assert player_entity.is_moving is True
        assert new_location[0] < player_entity.physical_location[0] < (dimensions[0] - 0.5)
